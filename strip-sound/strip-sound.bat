...
rem Parse each files
:next
for %%g in (%*) do (
	echo %%~g
	del /Q "%%~dpng-out.avi" 2> NUL

	%~dp0mencoder.exe -msglevel all=3:statusline=0 -nosound -ovc copy -of avi -o "%%~dpng-out.avi" -- "%%~g"
	...
	if not exist "%%~dpng-out.avi" (
		echo.
		echo Encoding %%~g failed; see above >&2
		call :end
	)
)
